			...
			?>
				<tr <?php if($message->read_status == 0) { ?> class="text-weight-bold" <?php } ?>>
					...
					<td width="20%">
						...
						data-toggle="tooltip" data-original-title="Mark as important"><i class="text-warning <?=($message->fav_inbox == 0 ? 'far fa-bell' : 'fas fa-bell')?>"></i></a><a href="<?=base_url('communication/mailbox/read?type=inbox&id='.$message->id)?>" class="text-dark mail-subj"><?='&nbsp;&nbsp;&nbsp;'.$this->application_model->getUserNameByRoleID($senderRoleID, $senderUserID)['name']?></a>
					</td>
					<td>
